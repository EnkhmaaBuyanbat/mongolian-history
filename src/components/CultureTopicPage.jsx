...
function CultureTopicPage({ topic }) {
  ...
  return (
    <article className="culture-page">
      ...
      <section className="culture-section culture-section-alt"><div className="section-inner"><div className="culture-heading"><p className="section-label">04 · {ui.evidence}</p><h2>{ui.evidence}</h2></div><CultureEvidenceCollection topic={topic} labels={ui} /></div></section>
      {relatedPeople.length ? <section className="culture-section"><div className="section-inner"><div className="culture-heading"><p className="section-label">05 · {ui.context}</p><h2>{ui.people}</h2></div><RelatedRecords records={relatedPeople.map((person) => getLocalizedPerson(person, peopleLocale))} label={ui.person} viewLabel={ui.viewRecord} /></div></section> : null}
      {relatedPlaces.length || relatedSites.length ? <section className="culture-section culture-section-alt"><div className="section-inner"><div className="culture-heading"><p className="section-label">06 · {ui.landscape}</p><h2>{ui.places}</h2></div><RelatedRecords records={[...relatedPlaces, ...relatedSites].map((record) => getLocalizedEntity(record, entityLocale))} label={ui.placeSite} viewLabel={ui.viewRecord} /></div></section> : null}
